...
(define (unique? str)
  (let ([strings (string-split str)])
    (= (length strings) (set-count (list->set strings)))))

(define (no-anagrams? str)
  (let* ([sort-string (lambda (s) (list->string (sort (string->list s) char<?)))]
         [sorted (string-join (map sort-string (string-split str)))])
    (unique? sorted)))

(define (step-1 passphrases)
  (length (filter unique? passphrases)))

(define (step-2 passphrases)
  (length (filter no-anagrams? passphrases)))

(define input '("bdwdjjo avricm cjbmj ran lmfsom ivsof"
